...
const EditStep1Schema = ListUpdateParamsSchema.pick({
	name: true,
	version: true,
	description: true,
	game: true,
...
type EditStep1FormData = z.infer<typeof EditStep1Schema>;

export function ListEditStep1() {
	const { data: games } = useGames();
	const formData = useListEditFormData();
	const { setFormData, setStep } = useListEditActions();

	const form = useForm<EditStep1FormData>({
		resolver: zodResolver(EditStep1Schema),
		defaultValues: {
			name: formData.name || "",
	...
	function onSubmit(data: EditStep1FormData) {
		setFormData(data);
		setStep(2);
	}
